...
function decodeEnvelope(text, limits, expectedAlgorithm) {
  ...
  if (lines.length < 6) fail('MALFORMED_SNAPSHOT', 'Snapshot envelope is truncated');
  const [magic, version, algorithm, metadataJson, payloadJson, checksum, ...extra] = lines;
  if (extra.length) fail('MALFORMED_SNAPSHOT', 'Snapshot contains unexpected trailing records');
...
async function writeAtomic(filePath, content, tempSuffix) {
  const target = safePath(filePath, limitsRef);
  const dir = path.dirname(target);
  const base = path.basename(target);
  await fs.mkdir(dir, { recursive: true });
  const temp = path.join(dir, `.${base}.${process.pid}.${tempSuffix}.tmp`);
  let committed = false;
  try {
...
function createSnapshotStore(config = {}) {
  ...
  limitsRef = validateLimits(config.limits ?? {});
  ...
  function create(payload, metadata = {}) {
    const envelope = buildEnvelope(payload, metadata, limitsRef, algorithm);
  ...
  async function save(filePath, payload, metadata = {}) {
    const target = safePath(filePath, limitsRef);
    let snapshot;
    try { snapshot = create(payload, metadata); } catch (error) { throw error; }
    try {
      const token = `${Date.now()}-${mutationSequence++}`;
      const dir = path.dirname(target);
      const base = path.basename(target);
      await fs.mkdir(dir, { recursive: true });
      const temp = path.join(dir, `.${base}.${process.pid}.${token}.tmp`);
      let committed = false;
      try {
  ...
  async function load(filePath) {
    ...
    try {
      ...
      return decodeEnvelope(content, limitsRef, algorithm);
    } catch (error) {
