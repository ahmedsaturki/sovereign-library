...
export class DirectoryWalkerError extends Error {
  constructor(code, message, details = {}) {
    super(message);
    this.name = 'DirectoryWalkerError';
    this.code = code;
...
function normalizeOptions(options = {}) {
  ...
  const integerFields = ['maxDepth', 'maxEntries', 'maxPathLength', 'maxNameLength', 'maxDirectoryEntries', 'maxSymlinkDepth', 'maxVisitedDirectories', 'maxWorkUnits'];
  ...
  if (out.deadlineMs !== null && (!Number.isFinite(out.deadlineMs) || out.deadlineMs <= 0)) fail('INVALID_OPTIONS', 'deadlineMs must be null or a positive number');
  if (out.includeSpecial !== true && out.includeSpecial !== false) fail('INVALID_OPTIONS', 'includeSpecial must be boolean');
...
function makeState(root, options, capabilities) {
  const start = capabilities.now ? Number(capabilities.now()) : performance.now();
  return { root, options, capabilities, start, work: 0, entries: 0, visitedDirectories: 0, stopped: false, partialFailure: null, activeDepth: 0 };
}

function now(state) { return state.capabilities.now ? Number(state.capabilities.now()) : performance.now(); }
...
function checkpoint(state, weight = 1) {
  ...
  if (state.capabilities.signal?.aborted) fail('ABORTED', 'traversal aborted');
}
...
function makeEntry(root, absolutePath, depth, type, metadata = {}) {
  const rel = relativePath(root, absolutePath);
  return Object.freeze({ path: rel, type, depth, ...metadata });
}
...
async function listChildren(state, node) {
  ...
  for (const child of raw) {
    ...
    normalized.push({ name, type, entry: child });
  }
...
async function inspectSymlink(state, absolutePath) {
  ...
  catch (error) { fail(error.code === 'ROOT_MISMATCH' ? 'ROOT_ESCAPE' : 'ROOT_ESCAPE', 'symlink resolves outside traversal root', { path: relativePath(state.root, absolutePath) }); }
...
export async function walk(root, options = {}, capabilities = defaultCapabilities) {
  ...
  const caps = Object.freeze({ ...capabilities, signal });
  const state = makeState(root, opts, caps);
  const result = [];
  const stack = [{ absolute: root, relative: '.', depth: 0, ancestry: new Set([root]) }];
  ...
  async function deliver(entry) {
    ...
    if (opts.mode === 'visitor') {
      if (typeof options.onEntry !== 'function') fail('INVALID_OPTIONS', 'visitor mode requires onEntry');
      try { await options.onEntry(entry); } catch (error) { fail('VISITOR_FAILURE', error instanceof Error ? error.message : String(error)); }
    } else {
      result.push(entry);
    }
  }
  ...
  try {
    while (stack.length) {
      ...
      for (let index = children.length - 1; index >= 0; index -= 1) {
        ...
        checkpoint(state);
        const absolute = join(node.absolute, child.name);
        ...
        if (child.type === 'directory') {
          const entry = makeEntry(root, absolute, node.depth + 1, 'directory');
          await deliver(entry);
    ...
    const output = opts.mode === 'visitor' ? Object.freeze({ mode: 'visitor', entries: state.entries, work: state.work }) : Object.freeze(result.map(item => Object.freeze({ ...item })));
    return output;
  } catch (error) {
