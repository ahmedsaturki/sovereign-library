...
function normalizeFields(fields) {
  ...
  const normalized = {};
  for (const [key, value] of Object.entries(input)) normalized[key] = key === 'error' ? normalizeError(value) : value;
  return normalized;
}
...
export class InMemorySink {
  ...
  write(record) { this.records.push(record); if (this.records.length > this.maxRecords) this.records.splice(0, this.records.length - this.maxRecords); }
...
export class ConsoleSink {
  constructor({ consoleLike = console } = {}) {
    if (!consoleLike || typeof consoleLike.error !== 'function' || typeof consoleLike.warn !== 'function' || typeof consoleLike.info !== 'function') throw new LoggerCubeError('INVALID_CONSOLE', 'consoleLike must provide error, warn and info');
...
export class Logger {
  constructor({ minLevel = 'info', sinks = [], context = {}, clock = { wallNow: () => new Date().toISOString(), monoNow: () => Number(process.hrtime.bigint()) / 1e6 }, maxRecordBytes = 64 * 1024, onSinkError = () => {} } = {}) {
    ...
    this.minLevel = minLevel; this.sinks = sinks.slice(); this.context = safeClone(context); this.clock = clock; this.maxRecordBytes = maxRecordBytes; this.onSinkError = onSinkError;
  }
  child(extraContext = {}) { return new Logger({ minLevel: this.minLevel, sinks: this.sinks, context: { ...this.context, ...safeClone(extraContext) }, clock: this.clock, maxRecordBytes: this.maxRecordBytes, onSinkError: this.onSinkError }); }
  enabled(level) { assertLevel(level); return LEVELS[level] >= LEVELS[this.minLevel]; }
  log(level, message, fields = {}) {
    assertLevel(level); if (!this.enabled(level)) return null;
    const record = { version: 1, ts: this.clock.wallNow(), monoMs: this.clock.monoNow(), level, message: String(message), context: safeClone(this.context), fields: safeClone(normalizeFields(fields)) };
    let encoded; try { encoded = JSON.stringify(record); } catch (error) { throw new LoggerCubeError('SERIALIZATION_FAILED', 'log record could not be serialized', { cause: error }); }
    if (Buffer.byteLength(encoded, 'utf8') > this.maxRecordBytes) throw new LoggerCubeError('RECORD_TOO_LARGE', 'log record exceeds configured size limit');
    for (const sink of this.sinks) { try { sink.write(record); } catch (error) { this.onSinkError(error, sink, record); } }
    return record;
  }
  ...
  async flush() { const results = []; for (const sink of this.sinks) if (typeof sink.flush === 'function') results.push(await sink.flush()); return results; }
  async close() { const results = []; for (const sink of this.sinks) if (typeof sink.close === 'function') results.push(await sink.close()); return results; }
}
