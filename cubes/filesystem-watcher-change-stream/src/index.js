...
export class FilesystemWatcherError extends Error {
  constructor(code, message) { super(message); this.name = 'FilesystemWatcherError'; this.code = code; Object.freeze(this); }
}
...
function immutable(value) { if (!value || typeof value !== 'object' || Object.isFrozen(value)) return value; for (const child of Object.values(value)) immutable(child); return Object.freeze(value); }
function normalizeRoot(input, index) { const root = text(input, `roots[${index}]`); const absolute = resolve(root); return { rootId: `root-${index + 1}`, path: absolute, publicRoot: absolute.split(sep).join('/') }; }
...
export function createWatcher(rawOptions) {
  ...
  async function startInjected(source) {
    ...
    for await (const raw of source) {
      safeData(raw, 'event'); const type = text(raw.type, 'event.type', 16); if (!TYPES.has(type)) fail('INVALID_EVENT', `unsupported event type ${type}`);
