...
3. If npm / PyPI were already updated:
...
4. Do NOT delete the Git tag. The tag's existence is the audit
   trail.

### Investigation

5. Pull the full GitHub Actions log for the affected workflow run:
   `gh run view <run-id> --log`.
6. Cross-reference every step against the expected sequence in
   `docs/DEPLOYMENT_RUNBOOK_V1.0.md`.
7. Check the OIDC token's `sub` and `aud` claims in the cosign
   signature (download the certificate from the Rekor log entry).
8. Inspect the SLSA attestation payload via
   `gh attestation verify --bundle-from-oci` and trace the
   materials / subjects to the workflow inputs.
9. If the root cause is a compromised GitHub Actions runner
   image, file an issue with GitHub support (`?report_abuse`) and
   freeze all workflow activity until the runner image is
   refreshed.
10. If the root cause is a compromised upstream action, the
...
11. Push a revert commit that removes the affected artefact URLs
   from the GitHub Release. Do NOT force-push — the original
   history must remain visible.
12. Re-run `prepare-authorized-release-artifacts.yml` from the
   last green commit. Verify every cosign signature and SLSA
   attestation before promotion.
13. Update `docs/release/` with an explicit `SECURITY_INCIDENT_*.md`
   record, including:
...
14. If npm / PyPI were affected, re-publish as `<cube>@0.1.1`
...
15. Check the GitHub status page (`githubstatus.com`) for runner
   outages. If active, the incident is external and the runbook
   reduces to "wait + post a status comment on the red PRs".
16. Check the upstream action vendors for reported outages
   (especially `gradle/actions/setup-gradle`,
   `android-actions/setup-android`).
17. Open an incident issue; assign to the on-call.
...
18. If the failure is in `vuln-trivy-*`: a new advisory may have
...
19. If the failure is in `gradle/actions/setup-gradle`: check
...
20. Capture the CVE in `docs/CVE_FEED_V1.0.md` with the affected
   dependency and the Sovereign surface area.
21. If the dep is direct, file a tracking issue with the
   `security` label and the remediation plan.
22. If the dep is transitive, verify Trivy actually flags the
   reachability path (false positives are common; the wave
   surfaces them so an analyst can review, not so they
   auto-block).
23. The hardening wave deliberately does not auto-bump
